...
#define ENCODER_THRESHOLD 14 // threshold for rate of change detection
...
void loop()
{
  ...
  if (encFlag)
  {
    ...
    if (abs(currentDelta) > ENCODER_THRESHOLD)
    {
      ...
      if (now - lastVibrationMs > REFRACTORY_MS)
      {
        iot.publishMsg("event/vibration", msg.c_str());
        lastVibrationMs = now;
      }
    }
  ...
  {
    const float R_RATIO = 2.5; // peak must be > R_RATIO * baseline
